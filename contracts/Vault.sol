...
contract Vault is Ownable {
  ...
  function setIntegrationProtocol(address[] calldata protocolAddresses, bool[] calldata statuses) external onlyOwner {
    require(protocolAddresses.length == statuses.length, "IntegrationProtocol: Invalid input length");
  ...
  function withdrawTokens(address[] calldata tokens) external onlyOwner {
    for (uint256 i = 0; i < tokens.length; i++) {
      ...
      if (tokens[i] == ETH_ADDRESS) {
        ...
        amount = IERC20(tokens[i]).balanceOf(address(this)) - totalAffiliateBalance[tokens[i]];
